...
def ex_search(keywords: str, page: int = 0) -> tuple:
    ...
    if page != 0:
        params['page'] = page
    html = httpx.get("https://exhentai.org", params=params, headers=headers, cookies=cookies).text
    res = re.findall(
...
def get_archive_html(url):
    return httpx.get(url, headers=headers, cookies=cookies).text


def get_download_page_url(archive_url: str) -> str:
    """
    提取下载界面的链接
    :param archive_url:画廊url
    :return: 选择下载画质的页面url
    """
    archive_text = get_archive_html(archive_url)
    ...
    time.sleep(2)
    httpx.get(download_choose_page_url, headers=headers, cookies=cookies)  # 正常走一下网页的流程,没有用
...
def get_download_zipfile_url(download_page_url: str, original: bool = False) -> str:
    ...
    :return:
    """
    download_url_data_last = httpx.post(download_page_url,
                                        data={
                                            "dltype": "org",
